...
##### 使用http2.0
- HTTP2 是基于帧的协议，每个帧都有表示帧长度的字段，相比http1.1的基于字节的协议来说解析速度要快。
- http2.0多路复用可在一个tcp连接中标记多个流的id来完成在一个tcp连接中完成多次http请求，多个连接的请求头和返回头可以乱序发送。
- 首部压缩，这个很好理解，http2.0会对请求头进行存储，在客户端和服务端存储一个叫做“首部表”的表结构，存储对应的请求头信息，多个请求到达时会先通过“首部表”来进行匹配，从而提高请求速度。
- http可以设置请求优先级
- 服务端推送
